...
class Params:
    ...
    def update_df(self): # called after every trial to update the DF
        ...
        self.trials_df.loc[new_index] = {
            ...
            'estim_params': np.nan if self.estim_params is None else self.estim_params,
            'circle_contrast': np.nan if self.circle_contrast is None else self.circle_contrast,
            'circle_radius': np.nan if self.circle_radius is None else self.circle_radius,
            'circle_startx': np.nan if self.circle_startx is None else self.circle_startx,
            'circle_starty': np.nan if self.circle_starty is None else self.circle_starty,
