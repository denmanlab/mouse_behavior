...
music = pyglet.resource.media('beep-03.wav')
...
task_monitor_plot = pyglet.window.Window(600, 300)
...
class Params():
    ...
    def save_params(self):
        df = pd.DataFrame(list(zip(self.stim_contrast,self.stim_orientation,self.stim_spatial_frequency,
                ...
                self.trial_AUTOREWARDED,self.trial_ONSET_REWARD,self.falsealarm,self.lapse,self.trialendtime)),
            columns =['Contrast', 'Orientation','Spatial Frequency','Delay','trialstarttime','stimontime','stimofftime','reactiontime','rewarded','rewardamount','rewardtime','AUTOREWARD','ONSET_REWARD','falsealarm','lapse','trialendtime'])
    ...
    def update_session_performance(self):
        # print(self.df.tail(1).stimontime)
        ...
        my_dpi = 82
        f, ax = plt.subplots(1,2,figsize=(600/my_dpi, 300/my_dpi))
        ax[0].plot(self.df['rewarded'].rolling(10,step=3).mean(),'g',label='rewarded')
        ax[0].plot(self.df['falsealarm'].rolling(10,step=3).mean(),'orange',label='false alarm')
        ax[0].plot(self.df['lapse'].rolling(10,step=3).mean(),'r',label='lapse')
        ax[0].set_xlabel('trial');ax[0].set_ylabel('rolling proportion of trials')
        ax[0].legend()

        ax[1].plot(self.df['rewarded'].cumsum(),'g',label='rewarded')
        ax[1].plot(self.df['falsealarm'].cumsum(),'orange',label='false alarm')
        ax[1].plot(self.df['lapse'].cumsum(),'r',label='lapse')
        ax[1].legend()
        ax[1].set_xlabel('trial');ax[1].set_ylabel('cumulative count of trials')
        plt.tight_layout()
...
def on_draw():   
    ...
    if not params.new_trial_setup: #check to see if we need to update params for a new trial. this only happens if the previous trial ended, and the animal hasn't licked for some amount of time that is currently hardcoded below. if not, execute the current trial. 
        if timer.time - params.lick_timestamps[-1] > 1.:
            setup_trial()
        else:
                ...
                if timer.time > params.trial_start_time[-1] + params.stim_delay[-1]:
                    ...
                    if not params.AUTOREWARD:
                       if check_lick_response():
                            print('lick detected')
                            reward =int(10 + 5 * params.stim_delay[-1])
...
def on_draw():
    ...
    sprite_progress = pyglet.sprite.Sprite(progress_image, x = 310, y = 0)
    sprite_progress.scale =0.8
...
def setup_trial():
    ...
    params.stim_delay.append(random.random() * 2. + 1. ) 
